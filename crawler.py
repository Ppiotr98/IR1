...
class LIFO_Policy:
    ...
    def updateURLs(self, c, retrievedURLs, retrievedURLsWD, iteration):
        ...
        tmp.sort(key = lambda url: url[len(url) - url[::-1].index('/'):])
...
def getNormalisedURLs(retrievedURLs):
    return [url.lower() for url in retrievedURLs]
    
#-------------------------------------------------------------------------  
# Remove duplicates (duplicates) (TODO)
def removeDuplicates(c, retrievedURLs):
    return [url.lower() for url in retrievedURLs if url.lower() not in c.URLs]
...
def getFilteredURLs(c, retrievedURLs):
    ...
    toLeft = [url for url in toLeft if url != c.toFetch]
    if c.debug:
